  ...
  return (
    <div ref={ref} className="export-target rounded-lg border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
        <div>
          <h3 className="text-sm font-semibold text-slate-700">Bazar Expenses</h3>
          <p className="text-xs text-slate-400">{expenses.length} entries this month</p>
        </div>
            ...
            <tbody>
              {expenses.map((e) => (
                <tr key={e.id} className="border-t border-slate-100">
                  <td className="px-4 py-2 text-slate-600">{prettyDate(e.date)}</td>
      ...
      <div className="export-hide border-t border-slate-200 p-3">
